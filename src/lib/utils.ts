...
export function getStandTypeName(standTypeId: string, standTypes: import("./constants").StandType[]): string {
  const stand = standTypes.find(s => s.id === standTypeId);
  return stand ? stand.name : "Unknown Stand";
}
